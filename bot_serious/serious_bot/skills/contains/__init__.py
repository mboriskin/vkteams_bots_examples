...
ALLOWED_PATTERN = r"^/contains ((IMA|IMIOS)-\d+) (\S+)$"
...
class Contains:
    ...
    def process(self, available_products, bot, chat_id_to_reply, jira_base_url, gitlab_base_url):
        ...
        if "IMA" in self.task_key:
            bot.send_text(
                chat_id=chat_id_to_reply,
                text=f"Проверка VK Teams Android: [{self.task_key}]"
                     f"({jira_base_url}/browse/{self.task_key})\n"
                     f"в релиз ветке [{self.release_branch}]"
                     f"(https://example.org)\n\n"
                     "в процессе... Ожидайте ⏳",
            ...
            asyncio.run(
                Android(product=available_products["vkt"]).check_release_branch_contains_task(
            ...
            bot.send_text(
                chat_id=chat_id_to_reply,
                text=f"Проверка VK Teams iOS: [{self.task_key}]"
                     f"({jira_base_url}/browse/{self.task_key})\n"
                     f"в релиз ветке [{self.release_branch}]"
                     f"(https://example.org)\n\n"
                     "в процессе... Ожидайте ⏳",
            ...
            asyncio.run(
                IOS(product=available_products["vkt"]).check_release_branch_contains_task(
                    jira_task_key=self.task_key,
                    release_branch=self.release_branch,
                    bot=bot,
                    chat_id_to_reply=chat_id_to_reply,
